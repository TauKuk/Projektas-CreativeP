...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    @isset($user->id)    
        ...
        <meta http-equiv='refresh' content="0; URL={{ url('/') }}/{{ Auth::user()->id }}/events">
        @endif
    @endisset
    ...
    <div id="app">
        <nav>
            <div class="header">
                ...
                @auth                                 
                    <a class="header-link underscore-animation" style="display:block;" href="/{{ Auth::user()->id }}/events">Renginiai</a>
                @endauth
                ...
                <div>
                    <div>
                        <!-- Right Side Of Navbar -->
                            ...
                            @guest
                                @if (Route::has('login'))
                                        <a class="header-link underscore-animation" href="{{ route('login') }}">{{ __('Prisijungti') }}</a>
                                @endif
                                
                                @if (Route::has('register'))
                                        <a class="header-link underscore-animation" href="{{ route('register') }}">{{ __('Registruotis') }}</a>
                                @endif
                            @else                                    
                                <ul class="nav-ul ">
                                    <div class="nav-listitem nav-animation">
                                        <div style="position:relative;">
                                        <li class="nav-animationline">
                                       
                                           {{ Auth::user()->name }}

                                            <ul class="nav-listitemdrop">
                                                <li>
                                                    <a class="underscore-animation" href="/users/{{ Auth::user()->id }}">
                                                        Vartotojo duomenys
                                                    </a>
                                                </li>

                                                <li>
                                                    <a class="underscore-animation" href="{{ route('logout') }}"
